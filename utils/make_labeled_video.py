...
def CreateVideo(clip,Dataframe,pcutoff,dotsize,colormap,DLCscorer,bodyparts2plot,cropping,x1,x2,y1,y2, vname):
        ...
        label_table = pd.read_csv('data_labels.csv')
        ...
        for index in tqdm(range(nframes)):
            ...
            for bpindex in range(len(bodyparts2plot)):
                if df_likelihood[bpindex,index] >= 0: #pcutoff:
                    ...
                    poses.extend([round((xc-xr) / x2, 3), round((yc-yr) / y2, 3)])
                    scores.append(round(df_likelihood[bpindex,index],3))
            ...
            # Create data format
            data.append({"frame_index": index + 1, 
                    "skeleton": [{"pose": poses, "score": scores}]})
        ...
        with open("data_json/{}.json".format(vname), "w") as write_file:
            ...
            if len(datanames)>0:
                ...
                if save_frames==True:
                    ...
                    if cropping:
                        ...
                        CreateVideo(clip,Dataframe,cfg["pcutoff"],cfg["dotsize"],cfg["colormap"],DLCscorer,bodyparts,cropping,x1,x2,y1,y2, vname) #NEED TO ADD CROPPING!
